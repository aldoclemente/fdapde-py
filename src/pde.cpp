...
#define pde_pybind_interface(LocalDim, EmbedDim, Order)                                                             \
      ...
      .def("get_quadrature_nodes" , &PDE_<LocalDim, EmbedDim, Order>::get_quadrature_nodes )                        \
      .def("get_dofs_coordinates" , &PDE_<LocalDim, EmbedDim, Order>::get_dofs_coordinates )                        \
...
PYBIND11_MODULE(_pde, mod){ 
    pybind11::class_<PDE_<2, 2, 1>>(mod, "cpp_pde_2_2_1") 
        pde_pybind_interface(2, 2, 1);

    pybind11::class_<PDE_<2, 2, 2>>(mod, "cpp_pde_2_2_2") 
        pde_pybind_interface(2, 2, 2); 

    pybind11::class_<PDE_<3, 3, 1>>(mod, "cpp_pde_3_3_1")
